      ...
      <View style={s.setInputs}>
        {(set.previousWeight || set.previousReps) && (
          <Text style={s.prevPerf}>
            Prev: {set.previousWeight || '—'}kg x {set.previousReps || '—'}
          </Text>
        )}
...
export default function WorkoutSessionScreen() {
  ...
  useEffect(() => {
    ...
    if (user?.id) {
      if (templateId) initFromTemplate(templateId);
      else initFromSplitBuilder();
    }
  }, [user?.id, templateId, sessionId]);

  const initFromSplitBuilder = async () => {
    setIsLoading(true);
    try {
      const todayIdx = new Date().getDay();
      const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
      const todayName = dayNames[todayIdx];
      const todayDay = days.find(d => d.dayName === todayName);

      if (!todayDay || todayDay.isRest || !todayDay.workoutName) {
        fetchTemplates();
        return;
      }

      const exercisesFromSplit = todayDay.sections.flatMap(s => s.exercises);
      if (exercisesFromSplit.length === 0) {
        fetchTemplates();
        return;
      }

      const name = todayDay.workoutName;
      const templateIdFromDay = todayDay.templateId;

      if (templateIdFromDay) {
        await initFromTemplate(templateIdFromDay);
      } else {
        ...
        const exRecords: ExerciseRecord[] = exercisesFromSplit.map((ex, idx) => ({
          id: `temp_${idx}_${Date.now()}`,
          exerciseId: ex.exercise_id || `temp_id_${idx}`,
          name: ex.name,
          notes: '',
          restTimeSeconds: ex.rest_time_seconds || 90,
          sets: Array.from({ length: ex.target_sets || 3 }, (_, si) => ({
            id: `temp_set_${idx}_${si}_${Date.now()}`,
            setNumber: si + 1,
            weight: '',
            reps: '',
            rir: '',
        ...
        startWorkout(session.id, user!.id, name, exRecords);
      }
    } catch (e: any) {
      console.error('Error initializing from split:', e);
      fetchTemplates();
  ...
  const fetchTemplates = async () => {
    ...
    try {
      ...
      if ((data || []).length > 0) {
        ...
        Alert.alert('No Workouts', 'No workout templates found. Build a split first.', [
  ...
  const initFromTemplate = async (idToUse: string) => {
    setIsLoading(true);
    try {
          ...
          {templates.length === 0 ? (
            ...
            templates.map((tpl) => (
              <TouchableOpacity
                ...
                onPress={() => { setShowTemplatePicker(false); initFromTemplate(tpl.id); }}>
